...
function createListElement() {
	...
	li.innerHTML = input.value + "<span style='cursor: pointer;'> 🗑️</span>"; // Create a text node for the list item text and append it to the list item
	...
	li.querySelector("span").addEventListener("click", removeListElement); // Add event listener to the span element in the list item for click event to remove the list item
}

function removeListElement(event) {
	event.target.parentNode.remove(); // Remove the parent node of the target element
}
